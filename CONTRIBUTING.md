...
[Lychee](https://github.com/lycheeverse/lychee) to check links, and
[Codespell](https://github.com/codespell-project/codespell) (`pip install codespell`)
to check spelling. Building slide decks or running `task run` also needs
...
Before opening a pull request, run `task lint` to check spelling and the built
site for broken links. Local lint does not build Slidev decks; CI does, using
Node 20.19.5. If Codespell flags a valid word, add it to the ignore list in
`.codespellrc`. If a URL is valid but fails in CI (CAPTCHA, bot blocking, login
wall), add it to the exclude list in `lychee.toml`.
