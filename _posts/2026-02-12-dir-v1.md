...
- [Key Strengths and Capabilities](#key-strengths-and-capabilities)
  ...
  - [Verifiable Claims and Trust](#verifiable-claims-and-trust)
...
### Verifiable Claims and Trust

While agent capabilities are often subjectively evaluated, DIR provides cryptographic mechanisms for data integrity and provenance tracking through **cryptographic signing and verification**.

DIR supports multiple trust models:

**Content Integrity**: All records are identified by their CID, enabling anyone to verify that content hasn't been tampered with by recomputing the hash.

**Author Attribution**: Records can be signed using private keys, proving authorship and enabling trust decisions based on publisher reputation.

**Trust Chains**: DIR supports local trust chain verification, allowing organizations to establish their own trust anchors and verification policies.

**Verification Metadata**: Records include verification details that can be queried and validated:

```bash
# Verify a record's signature
dirctl verify bafkreiabcd1234567890

# Output includes verification status and trust chain details
```

This verification framework allows users to make informed decisions about agent selection based on provenance, publisher identity, and trust policies specific to their use case, without requiring a centralized trust authority.
