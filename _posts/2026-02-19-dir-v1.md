...
In distributed agentic systems, trust is paramount. Agent Directory provides a comprehensive security model that combines content integrity and content provenance via the publisher's identity to enable secure agent discovery across organizational boundaries.
...
Agent Directory leverages [SPIFFE (Secure Production Identity Framework For Everyone)](https://spiffe.io/) and its runtime implementation [SPIRE](https://spiffe.io/docs/latest/spire-about/) to provide zero-trust security for the federation of different agent directory nodes. This enables:
